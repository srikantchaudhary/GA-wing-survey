...
export default function FormsList({ state, forms, customSections, onLogout }) {
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      // Don't fetch status if state is not available
      if (!state) {
        return;
      }
      ...
      await Promise.all(
        stateForms.map(async (f) => {
          try {
            ...
            // If 403 error, user needs to re-login
            if (err.status === 403) {
              onLogout();
            }
  ...
  // Filter out submitted forms - only show pending and draft forms
  const pendingForms = stateForms.filter(f => {
    const st = formStatus[f.id] || {};
    return !st.submitted;
  });

  // Get submitted forms for the sidebar
        ...
        <div className="w-80 border-l border-ga-border bg-white overflow-hidden flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto divide-y divide-ga-border">
            {submittedForms.map(f => {
              const st = formStatus[f.id] || {};
              return (
                <div key={f.id} className="p-4 hover:bg-[#F7F5EF] transition-colors">
